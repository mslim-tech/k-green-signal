# ui/rag.py
# -----------------------------------------------------------------------------
# 💬 AI에게 묻기 — 데이터 근거로 AI가 검색해 출처 인용 답변. '데이터 기반 제언'
...
# -----------------------------------------------------------------------------
# AI에게 묻기 화면 — 정형 데이터에서 검색 + 근거 인용 답변
# -----------------------------------------------------------------------------
def render_rag_tab(gate=None):
    st.subheader("💬 AI에게 묻기 — 데이터 근거로 답합니다")
    st.caption(
        "인지도 조사 데이터에서 찾아 **출처(파일·페이지)와 함께** 답합니다. "
        "근거가 없으면 지어내지 않고 “문서에서 찾을 수 없습니다”라고 답합니다."
    )
    ...
    mode_label = st.radio(
        ...
        help="**사실 인용** — 데이터에 있는 사실만 출처와 함께 답합니다.\n\n"
             "**데이터 기반 제언** — 3개년 추세를 근거로 제안합니다.\n\n"
             "📊 근거 사실은 인용으로, 💡 제언은 추론으로 나눠 표기합니다.",
    )
