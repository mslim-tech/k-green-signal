...
| 10 | eval 데이터 빵구 메우기(8/9→9/9) — 2023 환경표지_확대희망품목 1행만 추출 | **하이브리드 remediation 루프를 실데이터로 실증**: 원문 PDF p.67-68 직접 판독(표 3-52 1순위·표 3-53 복수응답) → 누락 6행을 `corrections.jsonl`(source-verify)로 확정·주입(2024/2025 라벨 일치, 페이지 정합 PDF 68) → build_all_chunks·Chroma 재인덱싱(218청크) → **eval 9/9(score 1.0)**. 부수 발견: 골드 `2023-greenmark-expand-top`의 기대 1위(유아·어린이 11.2%)가 원문과 어긋나(실제 1위 생활용품 74.1/50.6) 원문 진실로 정정. samples/(corrections·chunks·chroma) 전파로 클론 재현 유지 |
| 11 | (UI 폴리시, 실행 중 앱 스크린샷 기반) 좁은 사이드바에서 '다음 할 일'·'인덱스 정합'이 단어 중간에서 줄바꿈(대→시보드, 데→이터), '질의(Q&A)' 라벨·질의 화면 설명이 전문용어(RAG)로 딱딱 | 사이드바 두 박스를 마크다운 hard break로 '무엇을(굵게)+어떻게' 짧은 두 줄 구성(단어 잘림 해소, Playwright 스크린샷 검증). **'질의(Q&A)' → '💬 AI에게 묻기'로 개명**(모드 네비·화면 헤더·사이드바·캡션·E2E 일괄). 질의 화면 헤더 'RAG' 제거('데이터 근거로 답합니다'), 캡션·답변방식 툴팁을 굵은 라벨+줄바꿈으로 정리 |
