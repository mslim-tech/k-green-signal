...
- **커밋/푸시**: ~~이번 세션 산출물 + `samples/` 로컬 커밋 → `/ship`으로 푸시~~ → 커밋 완료·**푸시 보류(2026-07-03)**: `git push`가 403 — `researcherhojin` 계정에 `mslim-tech/k-green-signal` 쓰기 권한 없음(읽기만 가능). `feat/advise-signal-samples`에 main 대비 커밋들이 검증 완료 상태로 로컬 대기. 해결 경로: 오너의 collaborator(write) 초대 / 토큰 SSO 승인 / 포크→cross-repo PR / 다른 계정. 해결 후 `/ship` 재실행(멱등). (정책: [`DECISIONS.md` D15](./DECISIONS.md))
...
- **알려진 데이터 빵구(2026-07-03 eval 로 발견)** — **2023 `환경표지_확대희망품목`이 1행(생활용품 74.1, medium)만 추출**됨(2024·2025는 각 7행). 이 때문에 eval `2023-greenmark-expand-top`(기대: 유아·어린이 용품)이 실패 — **8/9 통과**(advise 신규 3건은 실 LLM 에서 전부 통과). 교정 경로는 원칙대로: `refill_vision`(비전 재판독) → 검수 화면에서 사람 확정 → 재인덱싱. 골드 케이스는 약화하지 않고 유지(데이터를 고친다).
