...
>
> 새 연도(예: 2026) 보고서를 **기존 std_id를 흔들지 않고 안전하게 추가**하는 로컬 절차는 [`ADD_YEAR.md`](./ADD_YEAR.md) 참고
> (전체 표준화 재실행 금지 → `integrate_oldyears` 증분 통합 경로).
>
...
>
> **UX/UI 개편 "결과 먼저, 관리 나중"(2026-07-03)**: 앱을 **3모드 IA**로 재구조화 — 🚦 대시보드(정형 CSV 있으면 **랜딩**, 키·인덱스 불필요) · 💬 질의 · 🛠 데이터 준비(기존 1~4단계 게이트 스텝퍼 유지). 정합 버그 5건 수정: 앱 재인덱싱 시 지식청크 소실(`build_chunks`→**`build_all_chunks`**)·CLI 막다른 안내 2곳·연도 필터 하드코딩·검수 큐 캐시 미무효화(mtime 키)·LLM 검증 키 게이트+새로고침 pid 복구(이중 과금 방지). 검수에 **순차 검수 모드**(저장→자동 다음 행, 안전 기본값 '원래 값 맞음')와 **원문 페이지 미리보기**(extract_vision 렌더러 재사용, PDF 없으면 폴백). advise 답변은 **헤딩 계약**(`#### KEEP/ADD/DROP/FIX`)+`parse_advise_sections`로 **갈래별 카드 렌더**(파싱 실패 시 원문 폴백), 출처는 카드+온디맨드 원문 페이지 토글. eval에 advise 골드 3케이스(6.8 백로그 해소). 이 시점 단위 **56** + E2E **21** = **77 passed**.
>
> **/ship 사전·적대 리뷰 라운드(2026-07-03, 같은 날 후속)**: 스페셜리스트 5 + 레드팀 + 적대 리뷰로 결함 추가 수정 — 검수 UI `llm_verified` 반영, **사람 우선 가드**(adjudicate 가 실행 중 사람 검수를 덮어쓰지 않음), **기각=`confirmed`**(skip 기록의 조용한 행 소실 방지), 빈 값 agree 에스컬레이션, QA 답변 세션 캐시(출처 토글의 유료 재생성 차단), `chunking.source_csv()` 호출 시점 해석(첫 세션 pre-dedup 색인 방지), adjudicate 과금 상한·큐 부재 크래시·모니터 rc 정직성, "검색은 키 없이" 문서 클레임 정정(임베딩에 키 필요). **회귀 테스트 18건 추가 → 단위 74 + E2E 21 = 95 passed**, ruff clean. 설계성 이연 8건은 아래 '리뷰 이연 항목'.
>
> ### 설계 원칙: "추측은 데이터가 아니다"
...
> - [x] 6.8 `eval/` — 검색·인용 품질 평가셋(`questions.jsonl` gold **9케이스** = cite 6 + advise 3, `run_eval.py` 채점 러너, mode 지원). 실 LLM **9/9(score 1.0)**. (advise 확충·2023 데이터 빵구 교정 완료 — 아래 참고)
...
- **커밋/푸시**: ~~이번 세션 산출물 + `samples/` 로컬 커밋 → `/ship`으로 푸시~~ → 커밋 완료·**푸시 보류(2026-07-03)**: `git push`가 403 — 현재 `gh` 인증 계정에 이 저장소 쓰기 권한 없음(읽기만 가능). `feat/advise-signal-samples`에 main 대비 커밋들이 검증 완료 상태로 로컬 대기. 해결 경로: 오너의 collaborator(write) 초대 / 토큰 SSO 승인 / 포크→cross-repo PR / 쓰기 권한 있는 계정. 해결 후 `/ship` 재실행(멱등). (정책: [`DECISIONS.md` D15](./DECISIONS.md))
...
- ~~**알려진 데이터 빵구(2026-07-03 eval 로 발견)** — 2023 `환경표지_확대희망품목`이 1행만 추출~~ → ✅ **해소(2026-07-03)**: 원문 대조 결과 두 층위였음 — ① **데이터**: 표 3-53(1+2+3순위 복수응답, PDF p.68)이 8행인데 텍스트 추출이 생활용품 74.1 **1행만** 잡음 → 원문 직접 판독 후 나머지 6행(개인위생 66.1·유아어린이 44.3·가전 44.0·가구 31.3·건축자재 27.1·서비스 13.1)을 `corrections.jsonl`(fixed, reviewer=human:source-verify)로 확정·주입, 2024/2025 라벨과 일치. ② **골드 오류**: 기대 1위 '유아·어린이 11.2%'는 **틀림** — 원문 1위는 생활용품(1순위 50.6% AND 복수응답 74.1% 둘 다), 유아·어린이는 3위값. 골드를 원문 진실(생활용품)로 **정정**(약화 아님 — 오기 교정, 사용자 문서 원칙과 일치). → **eval 9/9 (score 1.0)**. samples/(corrections·chunks·chroma)에도 전파해 클론 즉시 재현 유지.
- **리뷰 이연 항목(2026-07-03 /ship 레드팀·적대적 리뷰)** — 판단 필요/설계성이라 미수정, 기록만: ① **hybrid 게이트의 사람 승인 부재** — `llm_verified`가 사람 사인오프 없이 게이트를 통과·색인됨(샘플 corrections 17건도 전부 `llm:adjudicate`). '사람 승인 단계' 추가 여부는 명시적 결정 필요(Stage 3 후보). ② 브라우저 **탭 2개가 같은 인제스트 체인을 이중 구동** 가능(소유 토큰 없음 — 중복 과금·CSV 동시쓰기 위험). ③ 모드/단계를 벗어나면 fragment 미렌더로 **인제스트 체인 전진이 일시정지**(서브프로세스는 계속 돎, 표시 없음). ④ 복구 상태파일의 **pid 재사용 위험**(재부팅 후 무관 프로세스를 '진행 중'으로 오인/취소 가능 — 시작시각·토큰 저장 검토). ⑤ E2E가 `data/`는 미격리(클론·CI 에선 PDF 부재로 인제스트 E2E 전제 흔들림). ⑥ 성능: `validate_ready`+`load_rows`가 rerun 마다 전체 재계산(870행에선 무해, 데이터 성장 시 mtime 키 캐시 설계 필요). ⑦ 비전 후보의 raw 라벨이 std 라벨과 다르면 확정 시 이중 색인 가능(후보 생성 시 라벨 정규화 검토). ⑧ `signals.py`의 척도경계(2023→2024)·大변동(15%p) 상수는 데이터셋 특정 — 개편이 또 오면 curation 설정으로 이동.
- ~~**eval 확충** — gold 6케이스에 **외부 맥락·advise 상황 해석** 회귀 케이스 추가(6.8 백로그).~~ → ✅ 완료(2026-07-03): advise 골드 3케이스(2026 설계·척도 변경 FIX·그린워싱 외부맥락 인과 고지) + `run_eval.py` mode 지원. 실 LLM 검증은 `uv run python eval/run_eval.py`(과금).
- **테스트 커버리지 강화** — /ship 커버리지 감사 실측 **41%**(32 갭). E2E가 게이트 통과 픽스처라 **미도달 UI 흐름**: adjudicate 실행/키게이트/새로고침 복구, 비전 후보 확정·기각, 신호등 3단 분리(급변·해석 유의 expander), 랜딩-빈데이터-분기(prep), advise 파싱 실패 폴백(UI). E2E `data/` 격리(위 ⑤와 연동)로 클론·CI 인제스트 테스트 복구. 단위는 순수 함수(signals·gate·adjudicate·corrections) 이미 커버, 미검증은 대부분 Streamlit 상호작용 경로.
- **하우스키핑(소규모)** — `samples/outputs/chroma/*` 바이너리가 레퍼런스 갱신마다 git 이력을 불림(재빌드 시 segment UUID 변경). 재빌드는 `chunks.jsonl`로 결정적 재현 가능하므로, 향후 chroma를 Git LFS로 옮기거나 커밋 제외(부트스트랩이 chunks→index 재빌드) 검토.

> **다음 갈래 요약(2026-07-03 세션 종료 시점)** — **A(eval 빵구)** 완료. 남은 것: **B** 옛 연도(2014~2022) 인제스트(근본 해금, 옛 PDF 확보 gated) · **C** 리뷰 이연 견고성 8항목(위) · **D** 테스트 커버리지(위). 그리고 **푸시 권한 해결**이 전부의 랜딩 선결.
