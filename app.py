...
INDICATOR_CAVEATS: dict[str, str] = {
    ...
    # 아래 4개(범위A): 인지도 문항이 ~'22 4점 척도(전혀 모름/본 적 있음/조금 앎/잘 앎) →
    # '23 인지/비인지 2분할 → '24~ 2점 척도(알고 있음/모르고 있음)로 바뀐다(응답 라벨로 확인).
    # 원문 확증된 핵심 불연속은 '23→'24(2024 조사가 인지도 문항을 2점 척도로 전환) — 그 값만
    # 인용하고 '실제 변화가 아닐 수 있다'로 유의만 알린다(원인 단정·데이터 변경 없음).
    "환경성적표지_인지도": (
        "⚠️ 연도별 응답 척도가 달라 비교 유의: ~'22는 4점 척도"
        "(전혀 모름/본 적 있음/조금 앎/잘 앎), '23은 인지/비인지 2분할, "
        "'24~는 2점 척도(알고 있음/모르고 있음)로 문항이 바뀐다. "
        "'23→'24 하락(81.7→57.5)은 실제 인지 감소가 아니라 척도 변경 영향일 수 있다."
    ),
    "녹색매장_인지도": (
        "⚠️ 연도별 응답 척도가 달라 비교 유의: ~'22는 4점 척도"
        "(전혀 모름/본 적 있음/조금 앎/잘 앎), '23은 인지/비인지 2분할, "
        "'24~는 2점 척도(알고 있음/모르고 있음)로 문항이 바뀐다. "
        "'23→'24 하락(67.1→52.3)은 실제 인지 감소가 아니라 척도 변경 영향일 수 있다."
    ),
    "그린카드_인지도": (
        "⚠️ 연도별 응답 척도가 달라 비교 유의: ~'22는 4점 척도"
        "(전혀 모름/본 적 있음/조금 앎/잘 앎), '23은 인지/비인지 2분할, "
        "'24~는 2점 척도(알고 있다/모르고 있다)로 문항이 바뀐다. "
        "'23→'24 하락(77.1→62.6)은 실제 인지 감소가 아니라 척도 변경 영향일 수 있다."
    ),
    "저탄소제품_인지도": (
        "⚠️ 연도별 응답 척도가 달라 비교 유의: ~'22는 4점 척도"
        "(전혀 모름/본 적 있음/조금 앎/잘 앎), '23은 인지/비인지 2분할, "
        "'24~는 2점 척도(알고 있음/모르고 있음)로 문항이 바뀐다. "
        "'23→'24 하락(77.7→63.8)은 실제 인지 감소가 아니라 척도 변경 영향일 수 있다."
    ),
    # 범위B 추가: 위 4개와 척도 전환 구조는 같으나 '23→'24 하락폭이 작다(-9.4).
    "환경표지_인지도": (
        "⚠️ 연도별 응답 척도가 달라 비교 유의: ~'22는 4점 척도"
        "(전혀 모름/본 적 있음/조금 앎/잘 앎), '23은 인지/비인지 2분할, "
        "'24~는 2점 척도(알고 있다/모르고 있다)로 문항이 바뀐다. "
        "'23→'24 하락(90.7→81.3)은 실제 인지 감소가 아니라 척도 변경 영향일 수 있다."
    ),
}
