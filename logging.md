...
- **(a) 연속 추적 필터(즉효·결정적, 완료)**: `signals.compute_signals(min_coverage=)` + `dataset_years()`. `app.py` 신호등에 "연속 추적 항목만 (N개년 모두 값 있음)" 체크박스(기본 ON) + "📅 데이터 연도/표시 문항수" 캡션. 실측: 집계 up62/flat67/down86 → **연속만 14/22/28**, 가짜 큰 변동(53.5%→3.7%) 제거. 스샷 확인. `tests/test_signals.py` +1(min_coverage·dataset_years), **8 passed**.
