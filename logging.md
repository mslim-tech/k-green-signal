...
- **▶ 후속 결정(2026-06-28, 사용자)**: "신호등 항목이 너무 많고 불연속 2개년 항목의 가짜 큰 변동이 섞임 → **연도별 연속 추적 가능한 항목 위주로 정리 필요.**" 단, 이는 **문항(std_id)·응답라벨을 연도 간 일관되게 표준화하는 작업이 먼저**라고 판단 → 신호등 필터링은 그 표준화 재정리 이후로 **보류**. 지금은 다음 작업으로 진행.
