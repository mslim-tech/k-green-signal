...
- ⚠️ **남은 한계(정직)**: 과병합 52→51로 거의 안 줆 — (B)1순위/복수응답·(C)매트릭스(친환경고려_제품·추가지불의향)는 override로 못 풀고 **추출 단계에서 매트릭스 행을 별도 문항으로 쪼갠 구조** 문제라 별도 과제. 환경문제_관심도 2016=69(2017=88) 이상치는 매핑 아닌 추출/척도 차이 의심. 라벨 띄어쓰기 차(매우 관심있다↔매우 관심 있다)로 2014~15 일부 미연결. std_mapping.json은 gitignore(로컬).
