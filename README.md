...
│   ├── validate.py         # 준비 게이트(인덱싱 전 차단 — 빈/미확정/미검수)
│   ├── pipeline.py         # 인제스트 단계를 서브프로세스로 실행+로그 캡처
│   ├── chunking.py         # 6.1 청킹 (+출처 메타, corrections 반영)
│   ├── index.py            # 6.2 임베딩 · Chroma 인덱싱
│   ├── retriever.py        # 6.3 벡터 검색
│   └── answer.py           # 6.6 근거 인용 답변(+단계별 소요시간, RAG_FAKE_LLM 스텁)
├── tests/e2e/              # Playwright E2E(서버 자동기동·smoke·stepper·ingest·rag)
...
# 4) 준비 게이트 점검 → RAG 인덱스 구축 (청킹 → 임베딩 → Chroma)
uv run python rag/validate.py                                # 인덱싱 가능 여부 점검(차단 항목 표시)
uv run python rag/chunking.py && uv run python rag/index.py
uv run python rag/answer.py "2023년 확대 희망 친환경제품 1위는?"   # (선택) CLI 질의 테스트

# 5) 앱 실행 — 가이드 스텝퍼(업로드→인제스트→검수→인덱싱→질의)
