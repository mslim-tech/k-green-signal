...
| **+ 앱/검증** | `app.py` 가이드 스텝퍼 · `rag/curate/validate.py`(준비 게이트) · `rag/pipeline.py` · `tests/e2e`(Playwright) | 업로드→인제스트→검수→(게이트)→인덱싱→질의 + 🩺 시스템 로그 | ✅ |
...
│   ├── rag.py                      #   5단계 RAG 질의 탭(사실 인용/데이터 기반 제언 · 상세도 · 질문 재작성 · 예시 질문)
...
# 앱 실행 — 가이드 스텝퍼(업로드→인제스트→검수→인덱싱→질의)
